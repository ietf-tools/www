...
from ietf.utils.models import MenuItem, SubMenuItem
...
class MenuTests(TestCase, WagtailTestUtils):
    ...
    def test_menu_context_loads(self):
        ...
        self.assertEqual(menu_items[0], response.context['MENU'][0])
        self.assertEqual(menu_items[1], response.context['MENU'][1])
    
    def test_menu_in_template(self):
        self._build_menu()
        menu_items = MenuItem.objects.order_by("sort_order").all()
        response = self.client.get('/')
        self.assertContains(response, 'Menu Two'.format(menu_items[1].page.url), count=1)
        self.assertContains(response, 'Menu One'.format(menu_items[0].page.url), count=1)
